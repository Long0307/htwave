...
class InquiryController extends Controller
{
    ...
    public function store(InquiryStoreRequest $request): RedirectResponse
    {
        ...
        $contact = Contact::find(1)->toArray();vars: 
        ...
        [   
            ...
            'enquiries' => $validated['enquiries'],
        ];
        $user['to']=$contact['email'];
        Mail::send('mail',$data,function ($messages) use ($user){
                $messages->to($user['to']);
                $messages->subject('Thank You for Choosing HTWAVE, L.T.D. – 24/7 Support');
        });

        $inquiry = Inquiry::create($validated);

        return redirect()->back()->with('success', 'Thanks for your inquiry!');
    }
