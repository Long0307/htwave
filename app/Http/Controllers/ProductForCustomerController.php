...
class ProductForCustomerController extends Controller
{
    ...
    Solution $Solution): View
    {
        ...
        // Kiểm tra nếu Technology tồn tại
        if ($Technology) {
            // Chuyển thành mảng
            $Technology = $Technology->toArray();
        } else {
            // Nếu không tìm thấy Technology với id = 1, trả về mảng rỗng hoặc xử lý lỗi
            $Technology = [];
        }

        // dd($TechnologyCategory);
        // Trả về view với dữ liệu
        return view('product', compact('Technology','contact','Solution',
